...
djoralivsyni <- tbl(con, "djoralivsyni") %>% 
  left_join(tbl(con, "botnsyni"), by = "botn_id") %>% 
  left_join(tbl(con, "kanningarrapport"), by = "rap_id") %>% 
...
xmin <- min(min(alara1$ali_date), min(urslitsens1$synistoku_dato), min(urslitkemi$synistoku_dato))
xmax <- max(max(alara1$ali_date), max(urslitsens1$synistoku_dato), max(urslitkemi$synistoku_dato))
...
senskortfacet <- bakgrundskort_simp +
  ...
  facet_wrap_paginate(~kanning, ncol = colfacet, nrow = rowfacet, page = 1) +
