...
#litir
indexColourHex <- c(s4, s35, s3, s2, s1)
...
djoralivfacetfjord <- djoralivfjord +
  facet_wrap(~year) +
  ggtitle(paste0("Djóralívssýnini á alifjørði ", alifjord_nrr, " - Sýnini tey seinastu árini")) +
...
```{r djorfjordtogether, out.width='100%', fig.cap="Positiónirnar á djóralívssýnunum á alifjørðinum, øll árini"}
djoralivfjord
```
...
yvirlit <- djoralivsyni %>% 
  ...
  group_by(botn_id) %>% 
  mutate(grabbavidd_mean = mean(grabbavidd_m2.x[upptalt!="EINKI"], na.rm = TRUE),
         grabbavidd_sum = sum(grabbavidd_m2.x[upptalt!="EINKI"], na.rm = TRUE)) %>% 
  ungroup()
...
djoryvirlit_seinastakanning %>%
  ...
  kable_styling(bootstrap_options = c("condensed"), 
                full_width = F, font_size = 12) %>% 
  pack_rows(index = table(djoryvirlit_seinastakanning$svartalistin)) %>% 
  kable_styling(latex_options = c("hold_position"))
