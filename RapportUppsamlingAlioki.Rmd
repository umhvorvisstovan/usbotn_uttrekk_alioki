...
sum_fail <- yvirlit %>% 
  ...
  mutate(djorliv_id = paste0(botn_id, "-sum"))
...
index_seinastakanning %>% 
  select(-botn_id) %>% 
  mutate(`sum støða` = recode_factor(`sum støða`, !!i1navn := "A",
                                     !!i2navn := "B",
                                     !!i3navn := "C",
                                     !!i4navn := "D",
                                     !!i5navn := "E",
                                     !!index_godska_fail := "F")) %>% 
  mutate(`sum støða` = cell_spec(`sum støða`, color = "#FFFFFF00",
                                 background = map_chr(`sum støða`, stodafunc))) %>%
  rename(" " = `sum støða`) %>% 
  ...
  kable_styling(latex_options = c("hold_position")) %>% 
  footnote(symbol = "í síðstu kolonnu til høgru sæst støðan á grabbanum sum")
```
...
index_talva %>% 
  ...
  reactable(sortable = FALSE,
            ...
            columns = list(
              ...
              `sum støða` = colDef(minWidth = 150, sortable = TRUE),
