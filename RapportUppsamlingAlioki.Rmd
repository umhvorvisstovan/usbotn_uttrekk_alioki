...
djorsumclass <- djor %>%
  ...
  summarise(mean = mean(tal, na.rm = TRUE),
            sum = sum(tal, na.rm = TRUE))
...
djorsam_seinastakanning %>% 
  select(-botn_id) %>% 
  knitr::kable(caption = "Samanteljing", booktabs = T, align = c("l", "r", "r", "r", "r")) %>%
  kable_styling(bootstrap_options = c("condensed"), 
                full_width = F, font_size = 12) %>%
...
einfoldsum <- djor %>% 
  filter(!grabba_nr_nytt %in% c("sum", "mean")) %>% 
  group_by(botn_id, grabba_nr_nytt) %>%
  summarise(tal = sum(tal, na.rm = TRUE)) %>%
  ungroup() %>% 
  group_by(botn_id) %>% 
  mutate(mean = mean(tal, na.rm = TRUE),
         sum = sum(tal, na.rm = TRUE))

einfold <- djor %>% 
  group_by(djorliv_id) %>% 
  mutate(N = sum(tal, na.rm = TRUE),
         S = n()) %>% 
  ungroup() %>% 
  mutate(N_div_S = N/S,
...
einfoldtalva_seinastakanning %>% 
  select(-botn_id) %>% 
  knitr::kable(caption = "Einføld uppgerð", booktabs = T, align = c("l", "r", "r", "r", "r")) %>%
  kable_styling(bootstrap_options = c("condensed"), 
                full_width = F, font_size = 12) %>%
...
#nýggir listar
NSIlist <- sensmatch(djor, sensitivity, djorslag, ES100avg_2012)
ISIlist <- sensmatch(djor, sensitivity, djorslag, ES100min5_2012)
AMBIlist <- sensmatch(djor, sensitivity, djorslag, AMBI_EG)
AZTIlist <- sensmatch(djor, sensitivity, djorslag, AZTI)
...
urslitindex_long <- urslitindex %>% 
  ...
  mutate(stoda_godska = if_else(djorliv_id %in% godska_false$djorliv_id & !is.na(stoda), index_godska_fail, as.character(stoda)),
         stoda_godska = factor(stoda_godska, levels = index_stoda_na)) %>% 
  ungroup()
...
indextime <- urslitindex_long %>%
  ...
  geom_hline(yintercept = 0, show.legend = FALSE) +
  scale_shape_discrete(name = "Støðslag") +
  scale_colour_manual(values = c(indexColourHex, "grey50"), drop = FALSE, name = "Støða",
                      labels = c(index_stoda_na, "eingi markvirði ásett"))+
  guides(col = guide_legend(override.aes = list(shape = 15, size = 3)))+
  guides(shape = guide_legend(override.aes = list(size = 2))) +
...
indexkort <- function(dfkort, index){
  ...
  indexkortfacet <- bakgrundskort_simp +
    ...
    geom_point(data = dfkort,
               ...
               alpha = 0.8, size = stodd) +
    scale_colour_manual(values = c(indexColourHex, na_litur), drop = FALSE, name = "Støða",
                        labels = c(index_stoda_na, na_navn)) +
    scale_shape(name =  "Støðslag") +
    guides(col = guide_legend(override.aes = list(shape = 15, size = 3)))+
    guides(shape = guide_legend(override.aes = list(size = 2))) +
    facet_wrap(~kanning, ncol = colfacet, nrow = rowfacet) +
