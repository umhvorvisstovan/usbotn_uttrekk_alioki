...
index_stoda_na_reverse <- c(i1navn, i2navn, i3navn, i4navn, i5navn, index_godska_fail)
...
## Aliøki `r alioki` á alifjørði `r alifjord_nrr[[1]]` (`r alifjord_navn[[1]]`)
...
knitr::kable(tol1, caption = "Aling - Lyklatøl frá rakstri og forsøgn (1)", booktabs = T, align = c("l", "c", "c", "c", "r", "r")) %>% 
...
knitr::kable(tol2, caption = "Aling - Lyklatøl frá rakstri og forsøgn (2)", booktabs = T) %>% 
  kable_styling(bootstrap_options = c("condensed"), full_width = F) %>% 
  kable_styling(latex_options = c("hold_position"))
```
...
knitr::kable(tol3, caption = "Aling - Lyklatøl frá rakstri og forsøgn (3)", booktabs = T, align = c("l", "c", "r", "r", "r")) %>% 
  kable_styling(bootstrap_options = c("condensed"), full_width = F) %>% 
  kable_styling(latex_options = c("hold_position"))
```
...
knitr::kable(tol4, caption = "Aling - Lyklatøl frá rakstri og forsøgn (4)", booktabs = T) %>% 
  kable_styling(bootstrap_options = c("condensed"), full_width = F) %>% 
  kable_styling(latex_options = c("hold_position"))
```
...
- **Tal av alieindum:** Tal av alieindum sum eru registreraðar í dátugrunninum *usbotn* fyri ta ávísu kanningina.
...
talrap <- length(unique(dfkort$kanning))
...
# Tal av síðum sum eru neyðugar til paginate
n <- ceiling(talrap/(colfacet*rowfacet))

#work around, paginate riggar ikki um tað ikki er ein full rekja á síðstu síðu
x <- colfacet*rowfacet - (colfacet*rowfacet*n - talrap)
if(x > 0 & x < colfacet) {
  n <- n - 1
  rowfacet <- rowfacet + 1
}
...
```{r}
# filtrerað djóralívskaningar til djorlivyear
djoryear <- djortalva %>% 
...
ringar <- ringpos %>% 
  filter(stodslag_id == "RS") %>% 
  filter(year %in% dfkort$year) %>% 
  select(alioki_nr, alifjord_nr, stodslag_id, lat, lng, year, synistoku_dato)
...
djoralivalioki <- bakgrundskort_simp +
  ...
  geom_point(data = dfk,
             ...
             alpha = 0.8,
             size = stodd) +
  geom_point(data = ringar, aes(y = lat, x = lng, fill = "Ringsýni", shape = stodslag_id), size = 0.8) +
  coord_sf(xlim = c(min(dfk$lng, na.rm = TRUE), max(dfk$lng, na.rm = TRUE))+c(-0.005,0.005),
           ylim = c(min(dfk$lat, na.rm = TRUE), max(dfk$lat, na.rm = TRUE))+c(-0.001,0.001),
           expand = TRUE)+
  scale_color_discrete(name = "Sýnistøku ár") +
  scale_shape_discrete(name = "Støðslag") +
  scale_fill_manual(values = "black", name = "Ringsýni") +
  guides(col = guide_legend(override.aes = list(shape = 15, size = 3)))+
...
djoralivfacetalioki <- djoralivalioki +
  facet_wrap(~year) +
  geom_text(data = dfk, aes(y = lat, x = lng,
                            label = paste0(stodslag_id, "-", stodnr)),
            size = 2.5, check_overlap = FALSE, nudge_x = 0.004) +
...
yvirlit <- djoralivsyni %>% 
  ...
  arrange(djorliv_id) %>% 
  group_by(botn_id, year) %>% 
  nest() %>% 
  mutate(grabbar = map_dbl(data, nrow), 
         grabba_nr_nytt = map(grabbar, letterfunc)) %>% 
  unnest() %>% 
  ungroup() %>% 
  select(djorliv_id, botn_id, year, stodslag_id, grabba_nr, grabba_nr_nytt, grabbavidd_m2.x, upptalt,
         total_taxon_species, total_individuals_species, perc_individual_species) %>% 
  mutate(godska = if_else(upptalt == "FULT" & grabbavidd_m2.x == 0.100 & 
                            (perc_individual_species >= percentage_species & !is.na(perc_individual_species)), TRUE, FALSE)) %>% 
  group_by(botn_id, year) %>% 
...
# Sum grabbar, ið ikki lúka góðskukrøvini
sum_fail <- yvirlit %>% 
  group_by(botn_id) %>% 
  summarise(n = n(), 
            sumvidd = max(grabbavidd_sum), 
            godskasum = sum(godska)) %>% 
  mutate(godska = if_else(sumvidd == 0.2 & godskasum == 2, TRUE,
                          if_else(sumvidd == 0.1 & godskasum == 1, TRUE, FALSE))) %>% 
  mutate(djorliv_id = paste0(botn_id, "-tils"))
...
# grabbar, ið ikki eru upptaldir
ikkiupptalt <- yvirlit %>% 
  filter(upptalt == "EINKI") %>% 
  select(djorliv_id)
...
yvirlit %>% 
  # filtrerað til djorlivyear
  filter(year >= djorlivyear) %>% 
  select(-grabba_nr, -botn_id, -grabbavidd_mean, -grabbavidd_sum, -year) %>% 
  rename("grabbavídd (m2)" = grabbavidd_m2.x,
         ...
         "góðskukrøv" = godska) %>% 
  mutate(`grabbavídd (m2)` = cell_spec(`grabbavídd (m2)`, color = if_else(is.na(`grabbavídd (m2)`), s4,
                                                                          if_else(`grabbavídd (m2)` == 0.100,
                                                                                  s2, s4))),
         ...
         `góðskukrøv` = cell_spec(`góðskukrøv`, color = "white",
                                  background = if_else(is.na(`góðskukrøv`), s4,
                                                       if_else(`góðskukrøv` == TRUE, s2, s4)))) %>% 
...
all <- djor %>% 
  #tak grabbar út sum ikki eru upptaldir
  filter(!(djorliv_id %in% ikkiupptalt$djorliv_id)) %>% 
  filter(grabba_nr_nytt != "tils" & grabba_nr_nytt != "midal") %>% 
  group_by(botn_id) %>% 
  complete(grabba_nr_nytt, nesting(aphia_id, grabbavidd_mean, grabbavidd_sum)) %>% 
  mutate(tal0 = if_else(is.na(tal), 0, tal)) %>% 
  arrange(aphia_id, botn_id)


djorsums <- djor %>%
  right_join(all %>% select(botn_id, aphia_id, grabba_nr_nytt, grabbavidd_mean, grabbavidd_sum, tal0)) %>% 
  select(-djorliv_id, -grabba_nr_nytt) %>% 
  ungroup() %>% 
  group_by(botn_id, aphia_id, grabbavidd_sum, grabbavidd_mean) %>% 
  mutate(tils = sum(tal, na.rm = TRUE),
         tils = if_else(tils == 0, NA_real_, tils),
         midal = mean(tal0, na.rm = TRUE),
         midal = if_else(is.na(midal), NA_real_, midal)) %>% 
  filter(!is.na(tal)) %>% 
  select(-tal0) %>% 
  ungroup()
...
djor <- djor %>% 
  bind_rows(djorsums %>% 
              ...
              pivot_longer(cols = c(midal, tils), names_to = "grabba_nr_nytt", values_to = "tal") %>% 
              ...
              mutate(grabbavidd_m2.x = if_else(grabba_nr_nytt == "midal", min(grabbavidd_m2.x), max(grabbavidd_m2.x))) %>% 
              select(-test) %>% 
              ungroup() %>% 
              distinct_all()
  ) %>% 
...
djorsam_seinastakanning %>% 
  rename("miðal" = midal,
         "tils." = tils) %>% 
  select(-botn_id) %>% 
  knitr::kable(caption = "Samanteljing í høvuðsbólkar", booktabs = T, align = c("l", "r", "r", "r", "r", "r")) %>%
  kable_styling(bootstrap_options = c("condensed"), 
                full_width = F, font_size = 12) %>%
...
einfold <- djor %>% 
  ...
  group_by(djorliv_id) %>% 
  mutate(S = n(),
         N = round(sum(tal, na.rm = TRUE),0)) %>% 
  ungroup() %>% 
  mutate(N_div_S = round(N/S, 1),
...
perc_seinastakanning %>% 
  rename("miðal" = midal,
         "tils." = tils) %>% 
  select(-botn_id) %>% 
  knitr::kable(caption = "Fjølmentastu djórasløgini (%)", booktabs = T, align = c("l", "r", "r", "r", "r")) %>%
  kable_styling(bootstrap_options = c("condensed"), 
                full_width = F, font_size = 12) %>%
...
index_talva <- urslitindex_long %>%
  ...
  pivot_wider(id_cols = c(botn_id, index, navn),
              names_from = grabba_nr_nytt, values_from = c(index_value, stoda_godska), names_glue = "{grabba_nr_nytt}_{.value}") %>% 
  rename_at(vars(ends_with("_index_value")), funs(str_replace(., "_index_value", ""))) %>%
  rename_at(vars(ends_with("_godska")), funs(str_replace(., "_godska", ""))) %>%
  rename(`tils. støða` = tils_stoda) %>% 
  select(-ends_with("_stoda")) %>% 
...
numbercols <- ncol(index_seinastakanning) - 4

index_seinastakanning %>% 
  rename("miðal" = midal,
         "tils." = tils) %>% 
  select(-botn_id) %>% 
  mutate(`tils. støða` = recode_factor(`tils. støða`, !!i1navn := "A",
                                     !!i2navn := "B",
                                     !!i3navn := "C",
                                     !!i4navn := "D",
                                     !!i5navn := "E",
                                     !!index_godska_fail := "F")) %>% 
  mutate(`tils. støða` = cell_spec(`tils. støða`, color = "#FFFFFF00",
                                 background = map_chr(`tils. støða`, stodafunc))) %>%
  rename(" " = `tils. støða`) %>% 
  knitr::kable(caption = "Indexir - Vísitøl", booktabs = T,
               align = c("l", "l", rep("r", numbercols), "l"),
               escape = F) %>% 
  kable_styling(bootstrap_options = c("condensed"),
                full_width = F, font_size = 12) %>% 
  pack_rows(index = table(fct_inorder(index_seinastakanning$botn_id))) %>% 
  kable_styling(latex_options = c("hold_position")) %>% 
  footnote(symbol = "í síðstu kolonnu til høgru sæst støðan á samanlagda grabbanum (sum). Grái litur = líkur ikki góðskukrøv")
...
index_talva %>% 
  ...
  reactable(sortable = FALSE,
            ...
            columns = list(
              ...
              `tils. støða` = colDef(minWidth = 150, sortable = TRUE),
...
```{r}
# filtrerað djóralívskaningar til djorlivyear
djoryear <- urslitindex_long %>%
  filter(!grabba_nr_nytt %in% c("tils", "midal")) %>% 
...
#### Sensitivitetsindex, Indicator Species Index (ISI 2012), Norwegian Sensitivity Index (NSI) og Norwegian Quality Index 1 (NQI1)

```{r sensitivitetskort, out.width='100%', fig.cap="Indicator Species Index (ISI 2012), Norwegian Sensitivity Index (NSI) og Norwegian Quality Index 1 (NQI1)"}
if(max(djoryear)>=djorlivyear) {
...
dfwrite <- list(alara1, ali_lyklatol, urslitbotnl, urslitkemi, urslitsens1, 
                kemimean, sensmean, urslitdjoreinkul,
                urslitdjoraliv, urslitdjoraliv_altered, yvirlit, djoryvirlit,
                djorsam, einfoldtalva, perc,
                urslitindex_long, index_talva
)
dfname <- c("rakstrartol", "rakstrartol_lyklatol", "urs_botnlysing", "urs_kemi", "urs_sensorisk", 
            "urs_kemi_midal", "urs_sensorisk_midal", "urs_djoreinkul",
            "urs_djoraliv_raw", "urs_djoraliv_processed", "djoraliv_yvirlit", "djoraliv_djor",
            "djoraliv_sumclass", "djoraliv_einfold", "djoraliv_dominerandi",
            "djoraliv_index", "djoraliv_indextalva")
 ...
 * øll r-script liggja á https://github.com/umhvorvisstovan/usbotn_uttrekk_alioki

```{r}
# um Github repository er clona ístaðin fyri downloada, so ber til at síggja júst hvat fyri commit er brúkt til júst hesu rapportina!

if(dir.exists(".git")){
  commit <- system("git rev-parse HEAD", intern = TRUE)
  url <- str_remove(system("git config --get remote.origin.url", intern = TRUE), "\\.git")
  asis_output(paste0("* r-scriptini til hesa rapport liggja á ", url, "/tree/", commit))
}
```
