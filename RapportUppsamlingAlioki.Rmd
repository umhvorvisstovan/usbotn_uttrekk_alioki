...
indexColourHex <- c(i5, i4, i3, i2, i1)
...
```{r kortdjorfjordtogether, out.width='100%', fig.cap="Positiónirnar á djóralívssýnunum á alifjørðinum"}
...
djoralivfjord <- bakgrundskort_simp +
  ...
  coord_sf(xlim = c(min(dfk$lng, na.rm = TRUE), max(dfk$lng, na.rm = TRUE))+c(-0.005,0.005),
           ...
           expand = TRUE)+
  scale_color_discrete(name = "Sýnistøku ár") +
  ggtitle(paste0("Djóralívssýnini á alifjørði ", alifjord_nrr)) +
  guides(col = guide_legend(override.aes = list(shape = 15, size = 3)))+
  theme_void() +
  skrift + 
  bakgrundbla
...
djoralivfjord <- bakgrundskort_simp +
  ...
  coord_sf(xlim = c(min(dfk$lng, na.rm = TRUE), max(dfk$lng, na.rm = TRUE))+c(-0.005,0.005),
           ylim = c(min(dfk$lat, na.rm = TRUE), max(dfk$lat, na.rm = TRUE))+c(-0.001,0.001),
           expand = TRUE) +
  ggtitle(paste0("Djóralívssýnini á aliøki ", alioki)) +
  scale_color_discrete(name = "Støðslag") +
  guides(col = guide_legend(override.aes = list(shape = 15, size = 3)))+
...
# hetta tekur lana tíð!! skal møuliga filtrerast via urslitdjoraliv áðrenn...
...
djor <- yvirlit %>% 
  ...
  select(djorliv_id, botn_id, grabba_nr_nytt, grabbavidd_m2.x, class, genus, scientific_name, 
         rankname, aphia_id, tal, kanningy, svartalisti)

djor1 <- djor %>% 
  filter(kanningy == max(kanningy))
...
djoryvirlit <- djor1 %>% 
  pivot_wider(id_cols = c(botn_id, class, genus, scientific_name, rankname, aphia_id, svartalisti),
  ...
  mutate(svartalistin = if_else(svartalisti == FALSE, "sløg nýtt til index útrokningar",
                                "sløg á svartalista")) %>% 
  arrange(botn_id, class, scientific_name, svartalisti)

djoryvirlit %>% 
  select(-svartalisti, -svartalistin) %>% 
  knitr::kable(caption = "Úrslit rá seinastu djóralívskanning", booktabs = T,
               align = c("l", "l", "l", "l", "l", "r", "r", "r", "r")) %>% 
  kable_styling(bootstrap_options = c("condensed"), 
                full_width = F, font_size = 12) %>% 
  pack_rows(index = table(djoryvirlit$svartalistin)) %>% 
  kable_styling(latex_options = c("hold_position"))
...
urslitdjoraliv_sensi <- urslitdjoraliv_altered %>%  
...
urslitindex <- urslitdjoraliv_sensi %>% 
  ...
  mutate(S = map_dbl(data, taxa = aphia_id, sumTaxa),
         ...
         taxa_mangla_AMBI = S - map_dbl(data, index = azti_values, taxaIndex),
         AMBI = map_dbl(data, taxa = aphia_id, index = azti_values, count = tal, AMBI),
         NQI1 = map_dbl(data, taxa = aphia_id, index = azti_values, count = tal, NQI)
  ) %>% 
  select(-data) %>% 
...
index_keep_labels <- c("S", "N", "SN", "H'", "J'", "ES100", "ISI 2012", "NSI", "AMBI", "NQI1")
...
einfoldsums <- urslitindex_long %>%
  left_join(yvirlit, by = "djorliv_id") %>% 
  filter(index %in% index_einfold) %>% 
  group_by(botn_id, index) %>% 
  summarise(sum = sum(index_value, na.rm = TRUE),
            mean = mean(index_value, na.rm = TRUE))

einfold <- urslitindex_long %>%
  left_join(yvirlit, by = "djorliv_id") %>% 
  filter(index %in% index_einfold) %>% 
  pivot_wider(id_cols = c(botn_id, index, navn),
              names_from = grabba_nr_nytt, values_from = index_value) %>% 
  left_join(einfoldsums) %>% 
...
einfold %>% 
  ...
  kable_styling(bootstrap_options = c("condensed"),
                full_width = F, font_size = 12) %>% 
  pack_rows(index = table(einfold$botn_id)) %>% 
  kable_styling(latex_options = c("hold_position"))
```
...
indexkort <- function(dfkort, index){
  ...
  indexkortfacet <- bakgrundskort_simp +
    ...
    geom_point(data = dfkort,
               ...
               position=position_jitter(width=0.0008, height=0.0006),
               alpha = 0.8, size = stodd) +
    scale_colour_manual(values = indexColourHex, drop = FALSE, name = "Støða", labels = index_stoda) +
    scale_shape(name =  "Støðslag") +
    guides(col = guide_legend(override.aes = list(shape = 15, size = 3)))+
    guides(shape = guide_legend(override.aes = list(size = 2))) +
    facet_wrap(~kanning, ncol = colfacet, nrow = rowfacet) +
...
```{r shannonkort, out.width='100%', fig.cap="Sannon-Wiener (H')"}

indexkort(urslitindex_long, "H'")

```

#### Hurlberts diversitetsindex (ES100)

```{r hurlbertkort, out.width='100%', fig.cap="Hurlberts diversitetsindex (ES100)"}

indexkort(urslitindex_long, "ES100")

```

#### Indicator Species Index (ISI 2012)

```{r isikort, out.width='100%', fig.cap="Indicator Species Index (ISI 2012)"}

indexkort(urslitindex_long, "ISI 2012")

```

#### Norweian Sensitivity Index (NSI)

```{r nsikort, out.width='100%', fig.cap="Norweian Sensitivity Index (NSI)"}

indexkort(urslitindex_long, "NSI")

```

#### Norwegian Quality Index 1 (NQI1)

```{r nqikort, out.width='100%', fig.cap="Norwegian Quality Index 1 (NQI1)"}

indexkort(urslitindex_long, "NQI1")
